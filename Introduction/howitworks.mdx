...
<Tabs>
  <Tab title="Frontend">
    ...
    sequenceDiagram
        participant Client
        participant Wallet
        participant MagicBlockRPC as MagicBlock RPC
        participant SolanaConn as Solana Connection

        Client->>MagicBlockRPC: verifyTeeRpcIntegrity(challenge)
        MagicBlockRPC-->>Client: TDX quote
        Client->>Wallet: signMessage(challenge)
        Wallet-->>Client: signature
        Client->>MagicBlockRPC: getAuthToken(publicKey, signature)
        MagicBlockRPC-->>Client: auth token
        Client->>SolanaConn: connect ?token=authToken
    ```
  ...
  <Tab title="Program">
    ...
    sequenceDiagram
        ...
        User->>PDA: create PDA + deposit funds\n(set permissions)
        PDA->>PER: delegate access token scope
        User->>PDA: send encrypted request
        PDA->>Oracle: CPI signal to oracle program
        Oracle-->>PDA: process query + stream response chunks
        PDA-->>Frontend: emit history updates
        Frontend-->>User: render streaming response
    ```
  ...
  <Tab title="Oracles">
    ```mermaid
    flowchart TB
        subgraph HostCPU["Host CPU (Phala & Nillion)"]
            direction TB
            HostBase["Host OS / Hypervisor"]
            subgraph CPUTEE["CPU TEE (Confidential VM)"]
                direction TB
                GuestOS["Guest OS + runtime"]
                GPUDriver["GPU driver"]
                OracleRuntime["Loyal oracle runtime"]
            end
        end

        subgraph GPU["GPU"]
            direction TB
            GPUMgmt["GPU management firmware"]
            subgraph GPUTEE["GPU TEE (H100/H200)"]
                direction TB
                ComputeEngines["Compute engines"]
                HBM["Encrypted high-bandwidth memory"]
            end
        end

        OracleRuntime -->|Secure channel| ComputeEngines
        ComputeEngines -->|Encrypted results| OracleRuntime
        HostBase -->|Admin channel| GPUMgmt
        GPUMgmt -->|Provision & attest| ComputeEngines
        ComputeEngines -->|Integrity reports| GuestOS
    ```
